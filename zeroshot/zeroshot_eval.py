...
from finetune.finetune_dataset import getdataset as get_zero_dataset

co_embeddings = None
try:
    with open('../extra_graph/co_occurrence_new.pkl', 'rb') as f:
        co_embeddings = pickle.load(f)
        print(f"Loaded co_embeddings with shape: {co_embeddings.shape}")
except (FileNotFoundError, pickle.UnpicklingError) as e:
    print(f"Failed to load co-occurrence embeddings: {e}")
    co_embeddings = None  
...
def get_class_emd(model, class_name, co_embeddings_arg, device='cuda'):
    model.eval()
    with torch.no_grad():  # to(device=torch.device("cuda"iftorch.cuda.is_available()else"cpu"))
        ...
        for texts in tqdm(class_name):
            ...
            texts = model.module._tokenize(texts)  # tokenize
            class_embeddings = model.module.get_text_emb(texts.input_ids.to(device=device), texts.attention_mask.to(device=device))  # embed with text encoder
            # co_embeddings_arg = co_embeddings_arg.to(class_embeddings.device)
            # fuse_emb = model.module.fusion(class_embeddings, co_embeddings_arg)
            class_embeddings = model.module.proj_t(class_embeddings)  # embed with text encoder
...
def get_ecg_emd(model, loader, zeroshot_weights, device='cuda', softmax_eval=True):
    ...
    with torch.no_grad():
        for i, (ecg, target) in enumerate(tqdm(loader)):
            ...
            # predict
            ecg_emb = model.module.graph_encoder(ecg, mode='test')
            # ecg_emb = model.graph_encoder(ecg, mode='test')
...
def zeroshot_eval(model, set_name, device='cuda', args_zeroshot_eval=None):
    ...
    # get class embedding
    zeroshot_weights = get_class_emd(model, target_class, co_embeddings, device=device)
    # get ecg prediction
    pred = get_ecg_emd(model, test_dataloader,
